...
from scipy.integrate import simps  # SciPy < 1.11
...
class StructureMaskBuilder:
    """
    基于单个 RS Dataset + 已排序的 dcm_slices 缓存索引，
    用于高效、多次生成不同 ROI 的 3D mask。
    设计目标：
    - 输出与原 load_structure_mask 逻辑保持一致（fillPoly + round）
    - 避免每个 ROI / 每次调用都重复 dcmread + SOP UID 线性查找
    """

    def __init__(self, rs_ds, volume_shape, dcm_slices):
        self.rs = rs_ds
        self.volume_shape = volume_shape
        self.dcm_slices = dcm_slices
        # SOPInstanceUID -> slice_idx 映射（原逻辑里每个 contour 都线性扫描一次，这里优化为 O(1)）
        self.sop_uid_to_idx = {ds.SOPInstanceUID: i for i, ds in enumerate(dcm_slices)}
        # ROIName(lower) -> ROINumber
        self.roiname_to_num = {}
        if hasattr(self.rs, "StructureSetROISequence"):
            for roi in self.rs.StructureSetROISequence:
                if hasattr(roi, "ROIName") and hasattr(roi, "ROINumber"):
                    self.roiname_to_num[str(roi.ROIName).lower()] = int(roi.ROINumber)
        # ROINumber -> ROIContourSequence item
        self.roinum_to_contour = {}
        if hasattr(self.rs, "ROIContourSequence"):
            for c in self.rs.ROIContourSequence:
                if hasattr(c, "ReferencedROINumber"):
                    self.roinum_to_contour[int(c.ReferencedROINumber)] = c

    def build(self, structure_name="A") -> np.ndarray:
        num_slices, height, width = self.volume_shape
        mask_3d = np.zeros((num_slices, height, width), dtype=np.uint8)
        roi_num = self.roiname_to_num.get(str(structure_name).lower())
        if roi_num is None:
            return mask_3d
        roi_contour = self.roinum_to_contour.get(roi_num)
        if not roi_contour or not hasattr(roi_contour, "ContourSequence"):
            return mask_3d

        for contour_seq in roi_contour.ContourSequence:
            if not hasattr(contour_seq, "ContourData") or not hasattr(contour_seq, "ContourImageSequence"):
                continue
            ...
            slice_idx = self.sop_uid_to_idx.get(ref_sop)
            if slice_idx is None:
                continue

            ds_ref = self.dcm_slices[slice_idx]
            ...
            # row = (Y - origin_y) / row_spacing; col = (X - origin_x) / col_spacing
            poly_points = []
            for X, Y, Z in coords:
...
def load_structure_mask(rs_file, volume_shape, dcm_slices, structure_name="A"):
    if pydicom is None:
        raise ImportError('pydicom 未安装：无法读取 DICOM。请 pip install pydicom。')
    """
    ✅ 兼容旧接口：内部使用 StructureMaskBuilder（性能更好）。
    """
    rs_ds = pydicom.dcmread(rs_file)
    builder = StructureMaskBuilder(rs_ds, volume_shape, dcm_slices)
    return builder.build(structure_name=structure_name)
...
def gen_subroi_masks(mask_3d, ID, date_day, z, volume, ct_results, folder_name, base_roi, para_name, parameters,
    ...
    """
    ✅ 兼容旧接口：保留函数名与参数签名（避免你外部代码/旧脚本调用崩）
    - 内部改为：先纯函数派生 derive_subroi_masks_pure()，再统一写回 sub_images（保持你原 ct_results 结构）
    - 返回结构与旧版本完全一致：mask_dict[suffix][tag] = {'img':..., 'mask':...}
    """
    mask_dict = derive_subroi_masks_pure(
        mask_3d=mask_3d,
        ID=ID,
        date_day=date_day,
        z=z,
    ...
    # 写回 sub_images（保持你原行为）
    try:
        write_derived_subroi_subimages(ct_results, folder_name, base_roi, para_name, z, mask_dict)
    except Exception as e:
        # 若你某次不想存 sub_images，也可以把这里注释；默认保持原行为
        raise
    return mask_dict
...
class DebugConfig:
    ...
    out_dir: str = ""  # debug输出根目录
    save_npz: bool = True  # 保存npz(数组)
    save_png: bool = False  # 若你已有save_single_image/plt函数，可开启
    only_ids: set = field(default_factory=set)  # 只调试某些ID（空表示不过滤）
    only_rois: set = field(default_factory=set)  # 只调试某些ROI（空表示不过滤）
    only_z: set = field(default_factory=set)  # 只调试某些z（空表示不过滤）

    def allow(self, ID, roi, z):
        if not self.enabled:
            return False
        if self.only_ids and (ID not in self.only_ids):
            return False
        if self.only_rois and (roi not in self.only_rois):
            return False
...
class DebugDumper:
    ...
    def dump_npz(self, path, **arrays):
        if not self.cfg.enabled or not self.cfg.save_npz:
            return
        os.makedirs(os.path.dirname(path), exist_ok=True)
        # np.savez_compressed 会自动处理ndarray
    ...
    def dump_stage(self, folder_name, ID, date_day, roi, z, stage_name, **arrays):
        if not self.cfg.allow(ID, roi, z):
            return
        base = os.path.join(self.cfg.out_dir, folder_name, f"{ID}_{date_day}", roi, f"z{z:04d}")
        self.dump_npz(os.path.join(base, f"{stage_name}.npz"), **arrays)


# -------------------------
# 2) 数据对象：CTVolume / ROI / SliceContext
# -------------------------
@dataclass
class CTVolume:
    folder: str
    ...
    def z_pos_mm(self, z: int):
        return float(self.dcm_slices[z].ImagePositionPatient[2])
...
class ROIMask:
    name: str
    mask_3d: np.ndarray  # uint8 same shape as volume

    def has_any(self):
        return bool(self.mask_3d.any())

    def slice_mask(self, z: int):
        return self.mask_3d[z]
...
class SliceContext:
    ...
    z: int
    zcoord_cm: float
    roi_control_label: str

    # 图像与mask（原始）
    slice_img: np.ndarray
    slice_mask: np.ndarray
    slice_mask_control: np.ndarray
    ...
    def to_metric_context_dict(self):
        """
        保持与你现有 metric_* 函数兼容的 context 字典结构
        """
        return {
            ...
            'date': self.date,
            'z': self.z,
            'roi_control': self.roi_control_label,
            'zcoord': self.zcoord_cm,
            'slice_img': self.slice_img,
            'y_min_roi': self.y_min_roi,
            'y_max_roi': self.y_max_roi,
            'x_min_roi': self.x_min_roi,
            'x_max_roi': self.x_max_roi,
            'tag': self.tag,
            'derived_roi': self.derived_roi
        }


# -------------------------
# 3) 派生ROI注册机制（替代if-elif堆叠）
# -------------------------
@dataclass
class DerivedSpec:
    """
    描述一个派生ROI如何生成：
    - base_roi: 例如 'spc1' / 'rectum'
    - para_name: 例如 'spc1_for_MAS'
    - mask_tags: 例如 ('up','down','left','right')
    - extra_mask_name: 某些派生需要额外mask（例如rectum用spc1做特殊处理），这里用名字标识
    """
    base_roi: str
    para_name: str
    mask_tags: tuple
    extra_mask_name: str = ""  # e.g. 'spc1' when rectum needs spc1 mask


class DerivedRegistry:
    def __init__(self, para_mask_tag_map: dict):
        self.para_mask_tag_map = para_mask_tag_map
        self.specs: list[DerivedSpec] = []

    def register(self, base_roi: str, para_name: str, extra_mask_name: str = ""):
        tags = self.para_mask_tag_map.get(para_name)
        if tags is None:
            raise ValueError(f"⚠️ para_mask_tag_map 未定义派生ROI的tags: {para_name}")
        self.specs.append(DerivedSpec(base_roi=base_roi, para_name=para_name, mask_tags=tuple(tags), extra_mask_name=extra_mask_name))

    def iter_applicable(self, roi: str, parameters: dict):
        """
        返回当前roi下、且parameters里存在的派生spec
        """
        for s in self.specs:
            if s.base_roi == roi and (s.para_name in parameters):
                yield s


# =============================================================================
# 纯函数化派生 ROI（不写 ct_results）
# - derive_subroi_masks_pure(): 只负责从 base ROI 派生出子 ROI 的 img/mask 结构（与你原 gen_subroi_masks 输出一致）
# - write_derived_subroi_subimages(): 把派生 ROI 的 sub_images 写回 ct_results（保持你原结果结构）
# =============================================================================

@dataclass(frozen=True)
class ROIInstance:
    """一个可直接用于指标计算/存盘的 ROI 实例（slice 级）。"""
    roi_name: str  # 完整 ROI 名：f"{base_roi}_{suffix}_{tag}"
    base_roi: str  # 原始 ROI（如 spc1 / rectum）
    para_name: str  # 派生参数键（如 spc1_for_MAS）
    suffix: str  # 参数组合编码（你原 suffix_prefix）
    tag: str  # up/down/left/...（你原 mask_tags 的元素）
    z: int  # slice index
    img: np.ndarray  # float32 或原 dtype（这里保持你原逻辑：sub_img_d.astype(float32)）
    mask: np.ndarray  # uint8


def iter_derived_instances(base_roi: str, para_name: str, z: int, mask_dict: "OrderedDict") -> Iterable[ROIInstance]:
    """把 derive_subroi_masks_pure 的返回结构扁平化为 ROIInstance 序列。"""
    for suffix, region in mask_dict.items():
        if not isinstance(region, dict):
            continue
        for tag, payload in region.items():
            if not isinstance(payload, dict):
                continue
            if "img" not in payload or "mask" not in payload:
                continue
            roi_name = f"{base_roi}_{suffix}_{tag}"
            yield ROIInstance(
                roi_name=roi_name,
                base_roi=base_roi,
                para_name=para_name,
...
def write_derived_subroi_subimages(ct_results: dict, folder_name: str, base_roi: str, para_name: str, z: int,
                                   mask_dict: "OrderedDict") -> None:
    """保持你原行为：把派生 ROI 的 sub_images 存到 ct_results[folder][f'{base_roi}_{suffix}_{tag}']['sub_images'][z]"""
    for inst in iter_derived_instances(base_roi=base_roi, para_name=para_name, z=z, mask_dict=mask_dict):
        # 你原 save_subroi_to_ct_results 只写 sub_images，不写 mask；这里保持一致
        save_subroi_to_ct_results(ct_results, folder_name, base_roi, inst.suffix, inst.tag, z, inst.img, inst.mask)


def derive_subroi_masks_pure(
        mask_3d,
        ID,
        date_day,
        z,
    ...
    """纯函数版派生 ROI：不写 ct_results；返回结构与旧 gen_subroi_masks 一致。"""
    ...
    for one_param in margin_param_list:
        ...
        if para_name in ['spc1_for_MAS', 'spc1_for_LCR', 'HRCTV_for_LCR', 'rectum_for_LCR']:
            if 'spc1_for_MAS' == para_name:
                sub_img, slice_mask2 = correct_and_crop_region_for_cal_spc1_MAS(
                    folder_name, mask_3d, ID, date_day, z, volume,
                    slice_img, slice_mask, y_min_roi, y_max_roi, x_min_roi, x_max_roi, my, mx, My, Mx
                )
            else:
                sub_img, slice_mask2 = crop_subregion(
                    slice_img, slice_mask, y_min_roi, y_max_roi, x_min_roi, x_max_roi,
                    margin_y_min=my, margin_x_min=mx, margin_y_max=My, margin_x_max=Mx
                )
            ...
            for tag in mask_tags:
                if tag not in masks_dict:
                    continue
                m = masks_dict[tag]
                sub_img_d, mask_d = extract_roi_crop(ID, date_day, z, sub_img, m)
                subroi_dict[tag] = {
                    'img': sub_img_d.astype(np.float32),
                    'mask': mask_d.astype(np.uint8),
                }
            mask_dict[suffix_prefix] = subroi_dict

        elif 'rectum_dilated_upper_half' == para_name:
            slice_mask2 = generate_dilated_upper_half_mask(slice_mask, dilation_iter=my, propotion=mx)
            ...
            for tag in mask_tags:
                ...
                sub_img_d, mask_d = extract_roi_crop(ID, date_day, z, sub_img, slice_mask2)
                mask_d[:] = 1
                subroi_dict[tag] = {'img': sub_img_d.astype(np.float32), 'mask': mask_d.astype(np.uint8)}
            mask_dict[suffix_prefix] = subroi_dict

        elif 'rectum_ring_LCR' == para_name:
...
def _build_control_mask_and_layer_map_from_builder(builder: StructureMaskBuilder, structure_control):
    """
    与 _build_control_mask_and_layer_map 等价，但复用同一个 RS Dataset + 索引缓存（更快）。
    """
    volume_shape = builder.volume_shape
    if structure_control:
        mask_3d_control = np.zeros(volume_shape, dtype=np.uint8)
        layer_roi_map = {z: [] for z in range(volume_shape[0])}
        for roi_control in structure_control:
            try:
                mask_3d_sub = builder.build(structure_name=roi_control)
                for z in range(mask_3d_sub.shape[0]):
                    if np.any(mask_3d_sub[z]):
                        layer_roi_map[z].append(roi_control)
                mask_3d_control = np.logical_or(mask_3d_control, mask_3d_sub).astype(np.uint8)
            except Exception:
                pass
        return mask_3d_control, layer_roi_map
    else:
        layer_roi_map = {z: [] for z in range(volume_shape[0])}
        return None, layer_roi_map


# -------------------------
# 4) 核心：单文件夹处理（替代 cal_noref_metric_for_single_folder_ID_date_roi_metric 的屎山入口）
#    但不改你指标/掩膜/裁剪等已有函数
# -------------------------
def _build_control_mask_and_layer_map(rs_file, volume_shape, dcm_slices, structure_control):
    """
    复用你原有逻辑：生成 control mask + layer_roi_map
    """
    if structure_control:
        mask_3d_control = np.zeros(volume_shape, dtype=np.uint8)
        layer_roi_map = {z: [] for z in range(volume_shape[0])}
        for roi_control in structure_control:
            try:
                mask_3d_sub = load_structure_mask(rs_file, volume_shape, dcm_slices, structure_name=roi_control)
                for z in range(mask_3d_sub.shape[0]):
                    if np.any(mask_3d_sub[z]):
                        layer_roi_map[z].append(roi_control)
                mask_3d_control = np.logical_or(mask_3d_control, mask_3d_sub).astype(np.uint8)
        ...
        # 无控制结构时：layer map为空，control mask = 原mask（由外层赋值）
        return None, {z: [] for z in range(volume_shape[0])}


def cal_noref_metric_for_single_folder_refactored(
        idx,
        derived_registry: DerivedRegistry,
        parameters: dict,
        ct_folder: str,
        structure_name: list,
        structure_control: list,
        metrics: list,
        if_save_cal_image: int,
        img_save_folder: str,
        if_isolate_img_folder: int,
        debug_cfg: DebugConfig
):
    """
    与你原 cal_noref_metric_for_single_folder_ID_date_roi_metric 输出结构一致：返回 ct_results(dict)
    但：
    - 派生ROI通过 registry 统一生成
    - 中间变量可按需dump
    """
    dumper = DebugDumper(debug_cfg)
    ...
    # RS
    rs_file = glob.glob(os.path.join(ct_folder, "RS*.dcm"))[0]

    # CT
    ct = CTVolume.from_folder(ct_folder)
    # RS dataset + 缓存 builder（避免重复 dcmread / SOP UID 扫描）
    rs_ds = pydicom.dcmread(rs_file)
    builder = StructureMaskBuilder(rs_ds, ct.volume.shape, ct.dcm_slices)

    # (可选) 输出基本元数据，便于快速核对
    if debug_cfg.enabled:
        dumper.dump_json(os.path.join(debug_cfg.out_dir, folder_name, f"{ct.ID}_{ct.date_day}", "meta.json"), {
            "folder": folder_name, "ID": ct.ID, "date_day": ct.date_day, "date": ct.date,
            "spacing": ct.spacing.tolist() if hasattr(ct.spacing, 'tolist') else ct.spacing,
            "shape": list(ct.volume.shape), "origin": ct.origin.tolist() if hasattr(ct.origin, 'tolist') else ct.origin
        })
    folder_dict = ct_results.setdefault(folder_name, OrderedDict())
    folder_dict['ID'] = ct.ID
    folder_dict['date'] = ct.date

    # 预先缓存某些结构mask（按你原逻辑：spc1用于某些rectum派生的特殊处理）
    mask_cache = {}
    ...
    # 控制mask（与原逻辑一致）
    mask_3d_control_global, layer_roi_map = _build_control_mask_and_layer_map_from_builder(
        builder, structure_control
    )

    for roi in structure_name:
        # 原始roi mask
        mask_3d = builder.build(structure_name=roi)
        roi_mask = ROIMask(roi, mask_3d)

        # 缓存spc1（按你原逻辑）
        if 'spc1' not in mask_cache:
            mask_cache['spc1'] = builder.build(structure_name='spc1')

        # 如果没控制结构：control mask = 当前roi mask（与原逻辑一致）
        if mask_3d_control_global is None:
            mask_3d_control = mask_3d
        else:
            mask_3d_control = mask_3d_control_global

        # roi完全为空则跳过
        if not roi_mask.has_any():
            continue

        ct_results[folder_name].setdefault(roi, {})

        # z层循环
        for z in range(ct.num_slices):
            z_pos = ct.z_pos_mm(z)
            slice_mask = roi_mask.slice_mask(z)
            slice_mask_control = mask_3d_control[z]

            if not (np.any(slice_mask) and np.any(slice_mask_control)):
                continue
            ...
            ct_results[folder_name][roi].setdefault('zcoord', {})[z] = z_pos / 10.0  # cm

            # --- 获取slice与最小外接矩形（复用原函数） ---
            slice_img, y_min_roi, y_max_roi, x_min_roi, x_max_roi = get_slice_mask(z, ct.volume, slice_mask)
            dumper.dump_stage(folder_name, ct.ID, ct.date_day, roi, z, "slice_raw", slice_img=slice_img, slice_mask=slice_mask, slice_mask_control=slice_mask_control)

            # 裁剪最小外接矩形区域
            sub_img, slice_mask2 = crop_subregion(slice_img, slice_mask, y_min_roi, y_max_roi, x_min_roi, x_max_roi)
            dumper.dump_stage(folder_name, ct.ID, ct.date_day, roi, z, "sub_roi", sub_img=sub_img, slice_mask2=slice_mask2)

            # 保存用于全参考指标的sub_images（与你原逻辑一致）
            ct_results[folder_name][roi].setdefault('sub_images', {})[z] = sub_img.astype(np.float32)

            # dump中间变量（可选）
            dumper.dump_stage(folder_name, ct.ID, ct.date_day, roi, z, "01_base_crop",
                              slice_img=slice_img, slice_mask=slice_mask,
                              sub_img=sub_img, sub_mask=slice_mask2)

            # --- 派生ROI统一生成（替代一坨if roi==... and para in parameters） ---
            mask_dict_all = OrderedDict()
            for spec in derived_registry.iter_applicable(roi, parameters):
                add_para = None
                if spec.extra_mask_name:
                    add_para = mask_cache.get(spec.extra_mask_name, None)
                mask_dict_all[spec.para_name] = gen_subroi_masks(
                    mask_3d=mask_3d,
                    ID=ct.ID,
                    date_day=ct.date_day,
                    z=z,
                    volume=ct.volume,
                    ct_results=ct_results,
                    folder_name=folder_name,
                    base_roi=roi,
                    para_name=spec.para_name,
                    parameters=parameters,
                    slice_img=slice_img,
                    slice_mask=slice_mask,
                    y_min_roi=y_min_roi,
                    ...
                    x_min_roi=x_min_roi,
                    x_max_roi=x_max_roi,
                    mask_tags=list(spec.mask_tags),
                    add_para=add_para if add_para is not None else ''
                )

            # --- 额外保存用于人工质控的图像区域（与你原逻辑一致） ---
            sub_img_save, slice_mask_save = crop_subregion(
                slice_img, slice_mask, y_min_roi, y_max_roi, x_min_roi, x_max_roi,
                margin_y_min=70, margin_x_min=70, margin_y_max=70, margin_x_max=70
            )

            # --- 构造SliceContext（以后你想拓展任何环节都从ctx拿） ---
            ctx = SliceContext(
                folder_name=folder_name,
                ID=ct.ID,
                date_day=ct.date_day,
                ...
                roi=roi,
                z=z,
                zcoord_cm=z_pos / 10.0,
                roi_control_label=control_label,
                slice_img=slice_img,
                slice_mask=slice_mask,
                slice_mask_control=slice_mask_control,
                y_min_roi=y_min_roi,
                y_max_roi=y_max_roi,
                x_min_roi=x_min_roi,
                x_max_roi=x_max_roi,
                sub_img=sub_img,
                sub_mask=slice_mask2,
                spacing=ct.spacing,
                derived_roi="",
                tag="all"
            )
            ...
            for metric in metrics:
                ...
                # 原始roi
                if roi_allowed(metric_limit_dict, metric, roi):
                    result_dict = METRIC_FN[metric](ctx.to_metric_context_dict())
                    if result_dict:
                ...
                for para_key, one_mask_dict in mask_dict_all.items():
                    tags = derived_registry.para_mask_tag_map.get(para_key)
                    if tags is None:
                        raise ValueError(f"⚠️ 未知派生ROI tags: {para_key}")
                    ...
                    for suffix, region in one_mask_dict.items():
                        for tag in tags:
                            ...
                            dr_dict.setdefault('zcoord', {})[z] = z_pos / 10.0

                            ctx_d = copy.copy(ctx)
                            ctx_d.sub_img = img
                            ctx_d.sub_mask = mask
                            ctx_d.tag = tag
                            ctx_d.derived_roi = derived_roi

                            result_dict = METRIC_FN[metric](ctx_d.to_metric_context_dict())
                            if result_dict:
                                for k, v in result_dict.items():
                                    write_metric(ct_results, folder_name, derived_roi, k, z, v)

                    # 特殊组合指标（与你原逻辑一致：仅在tag循环之后重算）
                    if roi == 'spc1' and metric == 'nps' and (para_key in ['rectum_for_LCR', 'spc1_for_MAS', 'HRCTV_for_LCR', 'spc1_for_LCR', '']):
                        for suffix, region in one_mask_dict.items():
                            nps_ring_rl_num = combine_nps_metric(ctx.to_metric_context_dict(), region, suffix)
                            write_metric(ct_results, folder_name, roi, f'nps_ring_rl_{suffix}', z, nps_ring_rl_num)
            ...
            if if_save_cal_image == 1 and roi == 'rectum':
                ...
                if if_isolate_img_folder == 1:
                    save_single_image(
                        slice_mask_save, sub_img_save,
                        os.path.join(img_save_folder, os.path.basename(ct_folder), f'{ct.ID}_{ct.date_day}_{roi}_{z}.jpg'),
                        vmin, vmax
                    )
                else:
                    if add_tag != ct.date_day:
                        save_single_image(
                            slice_mask_save, sub_img_save,
                            os.path.join(img_save_folder, f'{add_tag}_{ct.ID}_{ct.date_day}_{roi}_{z}.jpg'),
                            vmin, vmax
                        )
                    else:
                        save_single_image(
                            slice_mask_save, sub_img_save,
                            os.path.join(img_save_folder, f'{ct.ID}_{ct.date_day}_{roi}_{z}.jpg'),
                            vmin, vmax
                        )
...
# -------------------------
# 5) 多文件夹并行处理（替代 process_multiple_ct_folders）
# -------------------------
def process_multiple_ct_folders_refactored(
        group_idx,
        para_mask_tag_map,
        parameters,
        ref_tag,
    ...
    if debug_cfg is None:
        debug_cfg = DebugConfig(enabled=False)
    ...
    # 派生ROI注册：把你原来的if-elif映射成“配置注册”
    registry = DerivedRegistry(para_mask_tag_map)

    # ✅ 这里注册的是“规则”，而不是写死在for z里
    # 你原逻辑中出现过的派生项在此登记即可（存在parameters里才会实际运行）
    registry.register(base_roi='spc1', para_name='spc1_for_MAS', extra_mask_name="")
    registry.register(base_roi='spc1', para_name='spc1_for_LCR', extra_mask_name="")
    registry.register(base_roi='HRCTV', para_name='HRCTV_for_LCR', extra_mask_name="")
    registry.register(base_roi='rectum', para_name='rectum_for_LCR', extra_mask_name="")
    registry.register(base_roi='rectum', para_name='rectum_dilated_upper_half', extra_mask_name="spc1")
    registry.register(base_roi='rectum', para_name='rectum_ring_LCR', extra_mask_name="spc1")

    print("▶️ 开始并行计算 ...")
    start_all = time.time()
    start_parallel = time.time()

    # 把 strict_metrics 传入子进程（避免改 cal_noref_metric_for_single_folder_engineered 的旧签名）
    setattr(cfg.debug, "strict_metrics", getattr(cfg.run, "strict_metrics", True))

    args_list = [
        (i, registry, parameters, ct_folder, structure_name, structure_control,
         metrics, if_save_cal_image, img_save_folder, if_isolate_img_folder, debug_cfg)
        for i, ct_folder in enumerate(ct_folders)
    ]

    with Pool(processes=cpu_count()) as pool:
        results = pool.starmap(cal_noref_metric_for_single_folder_refactored, args_list)
    ...
    compute_multi_reference_metrics_grouped(
        ct_results,
        roi_list=structure_name,
        ref_keywords=ref_tag,
        metrics=metrics
    )
...
# -------------------------
# 6) __main__ 替换调用示例（你只要替换一行）
# -------------------------
def _example_main_replacement(
        group_idx,
        para_mask_tag_map,
        parameters,
        ref_text,
    ...
    # 你可以把debug输出目录放到out_dir下面
    dbg = DebugConfig(
        enabled=False,  # ✅ 需要调试时改 True
        out_dir=os.path.join(out_dir, "_debug_dump"),
        save_npz=True,
        save_png=False,
        only_ids=set(),  # e.g. {"2024-09-24-006"}
        only_rois=set(),  # e.g. {"rectum","spc1"}
        only_z=set(),  # e.g. {73}
    )

    ct_results, img_save_folder = process_multiple_ct_folders_refactored(
        group_idx=group_idx,
        para_mask_tag_map=para_mask_tag_map,
        parameters=parameters,
        ref_tag=ref_text,
        if_isolate_img_folder=if_isolate_img_folder,
        if_save_cal_image=if_save_cal_image,
        out_dir=out_dir,
        ct_folders=ct_folders,
        structure_name=ROI,
        structure_control=roi_control,
        metrics=metric_list,
        debug_cfg=dbg
    )
    return ct_results, img_save_folder
...
@dataclass
class IOConfig:
    ct_folder_path_all: str
    out_dir: str
    out_dir_new_name: str = "0原始计算结果"
    data_prefix: str = "data"
    avg_prefix: str = "dataAvg"
    clear_out_dir_new: bool = True
...
class RunConfig:
    ...
    roi_control: Optional[List[str]] = None
    id_per_group: int = 200
    if_save_cal_image: int = 1
    if_isolate_img_folder: int = 0  # 你原逻辑是 ==1 才隔离；这里默认0更直观
    processes: Optional[int] = None  # None => cpu_count()

    # 指标异常处理：
    # - strict_metrics=True：保持你原始行为：任何指标报错会直接抛出（更安全，防止悄悄吞错）
    # - strict_metrics=False：指标报错会被捕获并跳过该指标（更“跑得完”），同时打印提示
    strict_metrics: bool = True
    continue_on_metric_error: bool = False  # 兼容项：若 True，会对单 slice 的异常做隔离并继续


@dataclass
class DerivationConfig:
    # parameters: para_name -> list of param tuples
    parameters: Dict[str, Any] = field(default_factory=dict)
    # para_mask_tag_map: para_name -> tags
    para_mask_tag_map: Dict[str, Tuple[str, ...]] = field(default_factory=dict)
    # 当某些派生需要额外mask（例如 rectum 某些派生需要 spc1），在这里配置
    extra_mask_name_map: Dict[str, str] = field(default_factory=lambda: {
        "rectum_dilated_upper_half": "spc1",
        "rectum_ring_LCR": "spc1",
    })
    # 如果你的 ROI 名包含下划线且不适用“取第一个下划线前缀”推断 base_roi，可在此显式指定
    base_roi_map: Dict[str, str] = field(default_factory=dict)
...
class AppConfig:
    io: IOConfig
    run: RunConfig
    derivation: DerivationConfig = field(default_factory=DerivationConfig)
    debug: DebugConfig = field(default_factory=lambda: DebugConfig(enabled=False))

    @staticmethod
    def from_file(path: str) -> "AppConfig":
        """支持 JSON / YAML（若安装 pyyaml）。"""
        ...
        with open(path, "r", encoding="utf-8") as f:
            if ext in [".yaml", ".yml"]:
                try:
                    ...
                    raise RuntimeError("未安装 pyyaml，无法读取 YAML。请 pip install pyyaml 或使用 JSON 配置。") from e
        ...
        return AppConfig(
            ...
            derivation=DerivationConfig(**cfg.get("derivation", {})),
            debug=DebugConfig(**cfg.get("debug", {})),
        )
        ...
        return {
            ...
            "derivation": {
                **self.derivation.__dict__,
                "para_mask_tag_map": {k: list(v) for k, v in self.derivation.para_mask_tag_map.items()},
            },
            "debug": self.debug.__dict__,
        }


def _infer_base_roi(para_name: str, base_roi_map: Dict[str, str]) -> str:
    if para_name in base_roi_map:
        return base_roi_map[para_name]
    # 默认：取第一个 "_" 前缀
    return para_name.split("_")[0]


def build_derived_registry_auto(para_mask_tag_map: Dict[str, Tuple[str, ...]],
                                parameters: Dict[str, Any],
                                extra_mask_name_map: Dict[str, str],
                                base_roi_map: Dict[str, str]) -> DerivedRegistry:
    registry = DerivedRegistry(para_mask_tag_map)
    for para_name in parameters.keys():
        if para_name not in para_mask_tag_map:
            # 参数里有但tag_map没定义，通常是配置遗漏
            print(f"⚠️ parameters 中存在 '{para_name}'，但 para_mask_tag_map 未定义其 tags，已跳过注册。")
            continue
        base_roi = _infer_base_roi(para_name, base_roi_map)
        extra = extra_mask_name_map.get(para_name, "")
        registry.register(base_roi=base_roi, para_name=para_name, extra_mask_name=extra)
    return registry


# =============================================================================
# 指标插件化（Metric Plugins）
# - 不改变原 metric_* 计算实现：默认插件仅做“适配/异常隔离/扩展钩子”
# - 未来新增指标只需：registry.register(FunctionMetricPlugin('xxx', metric_xxx))
# =============================================================================
...
class MetricPlugin:
    ...
    def postprocess(self,
                    ...
                    derived_by_para: Dict[str, "OrderedDict"[str, OneCTSeries]],
                    write_cb) -> None:
        """可选：在同一 slice 里，基于派生 ROI 的集合做额外组合指标（默认无）。"""
        return


class FunctionMetricPlugin(MetricPlugin):
    def __init__(self, name: str, fn, strict: bool = True):
    ...
    def compute(self, ctx: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ...
            if self.strict:
                raise
            print(f"⚠️ metric '{self.name}' 计算失败（已跳过）: {e}")
            return {}


class NPSMetricPlugin(FunctionMetricPlugin):
    """NPS 插件：保留原 metric_nps 计算，并在满足条件时追加组合指标 nps_ring_rl_{suffix}。"""

    def __init__(self, name: str, fn, strict: bool = True,
                 enable_combine: bool = True,
                 combine_base_roi: str = "spc1",
                 allowed_para_keys: Optional[Sequence[str]] = None):
        super().__init__(name=name, fn=fn, strict=strict)
        self.enable_combine = enable_combine
        self.combine_base_roi = combine_base_roi
        self.allowed_para_keys = set(allowed_para_keys or ['rectum_for_LCR', 'spc1_for_MAS', 'HRCTV_for_LCR', 'spc1_for_LCR', ''])

    def postprocess(self,
                    base_ctx: Dict[str, Any],
                    derived_by_para: Dict[str, "OrderedDict"],
                    write_cb) -> None:
        if not self.enable_combine:
            return
        roi = base_ctx.get("roi", "")
        if roi != self.combine_base_roi:
            return
        for para_key, one_mask_dict in (derived_by_para or {}).items():
            if para_key not in self.allowed_para_keys:
                continue
            # one_mask_dict: suffix -> region(tag->payload)
            for suffix, region in one_mask_dict.items():
                try:
                    nps_ring_rl_num = combine_nps_metric(base_ctx, region, suffix)
                except Exception as e:
                    if self.strict:
                        raise
                    print(f"⚠️ combine_nps_metric 失败（已跳过） para={para_key} suffix={suffix}: {e}")
                    continue
                write_cb(f"nps_ring_rl_{suffix}", nps_ring_rl_num)
...
class MetricRegistry:
    ...
    def select(self, names: Sequence[str]) -> List[MetricPlugin]:
        return [self.get(n) for n in names if n in self._plugins]


def build_default_metric_registry(strict: bool = True) -> MetricRegistry:
    reg = MetricRegistry()
    reg.register(FunctionMetricPlugin('mtf', metric_mtf, strict=strict))
    reg.register(FunctionMetricPlugin('nmtf', metric_nmtf, strict=strict))
    reg.register(FunctionMetricPlugin('zmtf', metric_zmtf, strict=strict))
    reg.register(FunctionMetricPlugin('radius', metric_radius, strict=strict))
    reg.register(FunctionMetricPlugin('diameter', metric_diameter, strict=strict))
    reg.register(FunctionMetricPlugin('static', metric_static, strict=strict))
    reg.register(FunctionMetricPlugin('nstatic', metric_nstatic, strict=strict))
    reg.register(FunctionMetricPlugin('zstatic', metric_zstatic, strict=strict))
    reg.register(NPSMetricPlugin('nps', metric_nps, strict=strict, enable_combine=True))
    return reg


def cal_noref_metric_for_single_folder_engineered(
        idx: int,
        derived_registry: DerivedRegistry,
        parameters: Dict[str, Any],
        ct_folder: str,
        structure_name: List[str],
        structure_control: Optional[List[str]],
        metrics: List[str],
        if_save_cal_image: int,
        img_save_folder: str,
        if_isolate_img_folder: int,
        debug_cfg: DebugConfig,
        processes_hint: Optional[int] = None,
):
    """工程化单 folder 计算：
    - 派生 ROI：纯函数 derive_subroi_masks_pure() + 独立 writer（保持原 ct_results 结构）
    - 指标：插件化 MetricRegistry（默认不改变 metric_* 算法实现）
    """
    dumper = DebugDumper(debug_cfg)
    ct_results = OrderedDict()
    ...
    # Debug: folder meta
    if debug_cfg.enabled:
        meta_path = os.path.join(debug_cfg.out_dir, folder_name, f"{ct.ID}_{ct.date_day}", "meta.json")
        dumper.dump_json(meta_path, {
            "folder": folder_name,
            "ct_folder": ct_folder,
            "ID": ct.ID,
            "date_day": ct.date_day,
            "date": ct.date,
            "shape": list(ct.volume.shape),
            "origin": ct.origin.tolist() if hasattr(ct.origin, "tolist") else ct.origin,
            "spacing": ct.spacing.tolist() if hasattr(ct.spacing, "tolist") else ct.spacing,
        })
    ...
    # control mask：同样复用 builder
    if structure_control:
        mask_3d_control = np.zeros_like(ct.volume, dtype=np.uint8)
        layer_roi_map = {z: [] for z in range(ct.volume.shape[0])}
        for roi_control in structure_control:
            try:
                m = builder.build(roi_control)
                for z in range(m.shape[0]):
                    if np.any(m[z]):
                        layer_roi_map[z].append(roi_control)
                mask_3d_control = np.logical_or(mask_3d_control, m).astype(np.uint8)
            except Exception:
                pass
    else:
        mask_3d_control = None
        layer_roi_map = {z: [] for z in range(ct.volume.shape[0])}

    # extra masks（派生依赖）
    extra_masks_needed = set(s.extra_mask_name for s in derived_registry.specs if s.extra_mask_name)
    extra_mask_cache: Dict[str, np.ndarray] = {}
    for extra_name in extra_masks_needed:
        try:
            extra_mask_cache[extra_name] = builder.build(extra_name)
        except Exception:
            extra_mask_cache[extra_name] = np.zeros_like(ct.volume, dtype=np.uint8)

    # metric plugins（默认不改算法；strict 由 cfg.run.strict_metrics 控制）
    # 说明：为避免修改旧的函数签名，这里从 debug_cfg 上读取 strict_metrics（由外层注入）
    strict_metrics = getattr(debug_cfg, "strict_metrics", True)
    metric_registry = build_default_metric_registry(strict=strict_metrics)
    selected_plugins = metric_registry.select(metrics)
    ...
    def _ensure_roi_meta(roi_name: str, z: int, zcoord_cm: float, control_label: str):
        d = ct_results[folder_name].setdefault(roi_name, OrderedDict())
        d.setdefault("roi_control", {})[z] = control_label
        d.setdefault("zcoord", {})[z] = zcoord_cm

    for roi in structure_name:
        try:
            mask_3d = builder.build(roi)
        except Exception:
            mask_3d = np.zeros_like(ct.volume, dtype=np.uint8)

        if mask_3d_control is None:
            mask_3d_control_roi = mask_3d
        else:
            mask_3d_control_roi = mask_3d_control
        ...
        for z in range(ct.num_slices):
            ...
            slice_mask_control = mask_3d_control_roi[z]
            ...
            ct_results[folder_name][roi].setdefault('roi_control', {})[z] = control_label
            ct_results[folder_name][roi].setdefault('zcoord', {})[z] = zcoord_cm

            slice_img, y_min_roi, y_max_roi, x_min_roi, x_max_roi = get_slice_mask(z, ct.volume, slice_mask)
            sub_img, slice_mask2 = crop_subregion(slice_img, slice_mask, y_min_roi, y_max_roi, x_min_roi, x_max_roi)
            ct_results[folder_name][roi].setdefault('sub_images', {})[z] = sub_img.astype(np.float32)

            dumper.dump_stage(folder_name, ct.ID, ct.date_day, roi, z, "01_sub_roi",
                              slice_img=slice_img, slice_mask=slice_mask,
                              sub_img=sub_img, sub_mask=slice_mask2,
                              slice_mask_control=slice_mask_control)

            # ---------- 派生 ROI（纯函数生成 + writer 写回 sub_images） ----------
            derived_by_para: Dict[str, OrderedDict] = OrderedDict()
            derived_instances: List[ROIInstance] = []

            for spec in derived_registry.iter_applicable(roi, parameters):
                add_para = ''
                if spec.extra_mask_name:
                    add_para = extra_mask_cache.get(spec.extra_mask_name, '')
                d = derive_subroi_masks_pure(
                    mask_3d=mask_3d,
                    ID=ct.ID,
                    date_day=ct.date_day,
                    z=z,
                ...
                derived_by_para[spec.para_name] = d

                # 保持你原行为：写 sub_images 到 ct_results
                write_derived_subroi_subimages(ct_results, folder_name, roi, spec.para_name, z, d)

                # 同时把 metadata 写齐（即使某些派生 roi 后续没有参与指标计算）
                for inst in iter_derived_instances(base_roi=roi, para_name=spec.para_name, z=z, mask_dict=d):
                    _ensure_roi_meta(inst.roi_name, z, zcoord_cm, control_label)
                    derived_instances.append(inst)
            ...
            base_ctx_dict = ctx.to_metric_context_dict()
            ...
            for plugin in selected_plugins:
                ...
                # base roi
                if roi_allowed(metric_limit_dict, metric_name, roi):
                    result_dict = plugin.compute(base_ctx_dict)
                    for out_name, v in (result_dict or {}).items():
                        write_metric(ct_results, folder_name, roi, out_name, z, v)

                # derived rois
                for inst in derived_instances:
                    derived_roi = inst.roi_name
                    if not roi_allowed(metric_limit_dict, metric_name, derived_roi):
                        continue

                    ctx_d = copy.copy(ctx)
                    ctx_d.sub_img = inst.img
                    ctx_d.sub_mask = inst.mask
                    ctx_d.tag = inst.tag
                    ctx_d.derived_roi = derived_roi

                    result_dict = plugin.compute(ctx_d.to_metric_context_dict())
                    for out_name, v in (result_dict or {}).items():
                        write_metric(ct_results, folder_name, derived_roi, out_name, z, v)

                # optional postprocess (e.g. nps combine)
                def _writer(extra_metric_name: str, val: Any):
                    write_metric(ct_results, folder_name, roi, extra_metric_name, z, val)

                plugin.postprocess(base_ctx_dict, derived_by_para, _writer)
...
def process_multiple_ct_folders_engineered(
        group_idx: int,
        cfg: AppConfig,
        ct_folders: List[str],
        derived_registry: DerivedRegistry,
):
    img_save_folder = os.path.join(cfg.io.out_dir, '1计算层面图像')
    ...
    args_list = [
        (i, derived_registry, cfg.derivation.parameters, ct_folder,
         cfg.run.roi_list, cfg.run.roi_control, cfg.run.metric_list,
         cfg.run.if_save_cal_image, img_save_folder, cfg.run.if_isolate_img_folder,
         cfg.debug, cfg.run.processes)
        for i, ct_folder in enumerate(ct_folders)
    ]

    nproc = cfg.run.processes or cpu_count()
    with Pool(processes=nproc) as pool:
        results = pool.starmap(cal_noref_metric_for_single_folder_engineered, args_list)
    ...
    compute_multi_reference_metrics_grouped(
        ct_results,
        roi_list=cfg.run.roi_list,
        ref_keywords=cfg.run.ref_keywords,
        metrics=cfg.run.metric_list
    )
    print(f'第{group_idx}组文件夹  全参考指标计算结束  {time.time() - start_ref:.2f} 秒   总耗时: {time.time() - start_all:.2f} 秒  开始保存结果')
...
def run_app(cfg: AppConfig):
    # parameters 优先级：如果你想保留“parameters_single 覆盖”的行为，可以在外部直接把 cfg.derivation.parameters 设为单一版本
    grouped_folders = group_ct_folders_by_id_only(cfg.io.ct_folder_path_all, id_per_group=cfg.run.id_per_group)

    # 自动注册派生 ROI（参数中出现的才注册）
    derived_registry = build_derived_registry_auto(
        para_mask_tag_map=cfg.derivation.para_mask_tag_map,
        parameters=cfg.derivation.parameters,
        extra_mask_name_map=cfg.derivation.extra_mask_name_map,
        base_roi_map=cfg.derivation.base_roi_map
    )

    out_dir_new = os.path.join(cfg.io.out_dir, cfg.io.out_dir_new_name)
    os.makedirs(out_dir_new, exist_ok=True)

    if cfg.io.clear_out_dir_new:
        for file_name in os.listdir(out_dir_new):
            fp = os.path.join(out_dir_new, file_name)
            if os.path.isfile(fp):
                os.remove(fp)

    # 打印参数组大小（对齐你原行为）
    for key, value in cfg.derivation.parameters.items():
        try:
            print(f"本次计算的 {key} 参数组大小：{len(value)}")
        except Exception:
            print(f"本次计算的 {key} 参数组大小：<unknown>")

    for group_idx, ct_folders in enumerate(grouped_folders, start=1):
        print(f"\n🚀 正在处理第 {group_idx}/{len(grouped_folders)} 组，共 {len(ct_folders)} 个文件夹...")
        ct_results, img_save_folder = process_multiple_ct_folders_engineered(
            group_idx=group_idx,
            cfg=cfg,
            ct_folders=ct_folders,
            derived_registry=derived_registry
        )
        save_ct_results_to_excel(group_idx, ct_results, out_dir_new, cfg.io.data_prefix, cfg.io.avg_prefix)

    merge_excel_results(cfg.io.out_dir, out_dir_new, prefix=cfg.io.data_prefix, avg_prefix=cfg.io.avg_prefix)


def _print_cfg_summary(cfg: AppConfig):
    print("\n========== 配置摘要 ==========")
    print("CT 根目录:", cfg.io.ct_folder_path_all)
    print("输出目录:", cfg.io.out_dir)
    print("ROI:", cfg.run.roi_list)
    print("控制ROI:", cfg.run.roi_control)
    print("指标:", cfg.run.metric_list)
    print("参考关键词:", cfg.run.ref_keywords)
    print("ID每组:", cfg.run.id_per_group, "并行进程:", cfg.run.processes or cpu_count())
    print("派生参数 keys:", list(cfg.derivation.parameters.keys()))
    print("Debug:", "ON" if cfg.debug.enabled else "OFF", "dir:", cfg.debug.out_dir)
    print("==============================\n")
...
def run_unit_tests() -> int:
    import unittest
    # =============================================================================
    # 内置单元测试（不依赖真实 DICOM/RS 数据）
    # - 让你能快速验证“重构是否破坏基础工具/接口”
    # - 不追求严格验证医学意义（真实数值一致性需要你自己的回归数据）
    #   python ct_metrics_engineered_v2.py --run-tests
    # =============================================================================
    class TestCoreUtils(unittest.TestCase):
        def test_crop_subregion_shape(self):
            img = np.zeros((10, 10), dtype=np.float32)
            mask = np.zeros((10, 10), dtype=np.uint8)
            mask[3:7, 2:8] = 1
            sub_img, sub_mask = crop_subregion(
                img, mask, 3, 7, 2, 8,
                margin_y_min=1, margin_x_min=1, margin_y_max=1, margin_x_max=1
            )
            self.assertEqual(sub_img.shape, (6, 8))
            self.assertEqual(sub_mask.shape, (6, 8))

        def test_extract_roi_crop(self):
            img = np.arange(100).reshape(10, 10)
            mask = np.zeros((10, 10), dtype=np.uint8)
            mask[4:6, 5:9] = 1
            sub_img, sub_mask = extract_roi_crop("T", "0", 0, img, mask)
            self.assertEqual(sub_img.shape, (2, 4))
            self.assertTrue(sub_mask.all())

    class TestDerivedROIPure(unittest.TestCase):
        def test_derive_rectum_ring_LCR(self):
            H, W = 128, 128
            slice_img = (np.random.RandomState(0).randn(H, W) * 10).astype(np.float32)
            slice_mask = np.zeros((H, W), dtype=np.uint8)
            slice_mask[40:90, 40:90] = 1

            ys, xs = np.where(slice_mask > 0)
            y_min, y_max = int(ys.min()), int(ys.max() + 1)
            x_min, x_max = int(xs.min()), int(xs.max() + 1)

            parameters = {"rectum_ring_LCR": [[2, 2, 0, 0, 0]]}  # 后三位无意义，保持格式
            d = derive_subroi_masks_pure(
                mask_3d=np.zeros((1, H, W), dtype=np.uint8),
                ID="T",
                date_day="0",
                z=0,
                volume=np.zeros((1, H, W), dtype=np.int16),
                folder_name="TEST",
                base_roi="rectum",
                para_name="rectum_ring_LCR",
                parameters=parameters,
                slice_img=slice_img,
                slice_mask=slice_mask,
                y_min_roi=y_min,
                ...
                x_min_roi=x_min,
                x_max_roi=x_max,
                mask_tags=["rectum_ring_LCR"],
                add_para=np.zeros((1, H, W), dtype=np.uint8)  # spc1 mask 占位
            )
            self.assertIsInstance(d, OrderedDict)
            self.assertTrue(len(d) >= 1)
            found = False
            for suffix, region in d.items():
                for tag, payload in region.items():
                    found = True
                    self.assertIn("img", payload)
                    self.assertIn("mask", payload)
                    self.assertGreater(payload["mask"].sum(), 0)
            self.assertTrue(found)

    class TestMetricPluginsSmoke(unittest.TestCase):
        def test_plugins_do_not_crash_in_nonstrict(self):
            reg = build_default_metric_registry(strict=False)
            H, W = 128, 128
            img = (np.random.RandomState(1).randn(H, W) * 10).astype(np.float32)
            ...
            for name in ["mtf", "nmtf", "zmtf", "radius", "diameter", "static", "nstatic", "zstatic", "nps"]:
                plugin = reg.get(name)
                out = plugin.compute(ctx)
                self.assertIsInstance(out, dict)

    suite = unittest.TestSuite()
    loader = unittest.TestLoader()
    suite.addTests(loader.loadTestsFromTestCase(TestCoreUtils))
    suite.addTests(loader.loadTestsFromTestCase(TestDerivedROIPure))
    suite.addTests(loader.loadTestsFromTestCase(TestMetricPluginsSmoke))
...
def _default_config_like_original() -> AppConfig:  # 修改参数配置的默认位置
    """给一个和你原 main 尽量接近的默认配置（路径需要你自己改）。"""
    return AppConfig(
        # 一、IO 与路径设置
        io=IOConfig(
            # 包含多个 CT 子文件夹的根目录，每个子文件夹应包含：CT*.dcm + RS*.dcm
            ct_folder_path_all=r"C:\Users\99563\Desktop\data",
            # 输出根目录（会在其中生成 csv、debug、图像等）
            out_dir=r"D:\ob仓库\研究\2工作\临床与科研工作\999后装施源器伪影研究+SPC\1分析\7-1SPC监测伪影和勾画\0指标筛选",
            # 是否在每次运行前清空原始计算结果 目录
            clear_out_dir_new=True,
            data_prefix="data",  # 输出数据文件名字
            avg_prefix="dataAvg",  # 输输出平均各层面结果的数据文件名字
            out_dir_new_name="0原始计算结果"  # 输出文件夹名字
        ),
        # 二、运行逻辑控制（你最常改的）
        run=RunConfig(
            # 要计算的指标名称（必须在 MetricRegistry 中存在）
            # 常见：nps, mtf, zmtf, nmtf, static, radius, diameter
            metric_list=['nps'],
            # 要处理的 ROI
            roi_list=['spc1'],
            # 控制 ROI：只有在该 ROI 存在的层面才计算
            # None / [] 表示不做层面过滤
            roi_control=['HRCTV'],
            # 用于全参考指标的“参考图像标识”
            # 来自文件夹名最后一段（如 ID_date_ori）
            ref_keywords=['ori'],
            # 是否保存“计算层面图像”
            if_save_cal_image=11,
            # 是否进一步给每个保存文件夹单独建图像子目录
            if_isolate_img_folder=11,
            # 每组包含多少个 ID（用于并行分批，防止内存爆）
            id_per_group=200,
            # 并行进程数（None = cpu_count）
            processes=None,
        ),
        # 三、派生 ROI 参数（高级 / 研究用）
        derivation=DerivationConfig(  # 派生 ROI 的几何参数# key = 派生 ROI 名称 # value = 参数列表，每一项含义由 gen_subroi_masks 定义
            # 手动给定派生参数 放到parameters={}里面
            # 'spc1_for_MAS': [[9, 3, 9, 3, 1, 'param_150']]
            # 或者自动批量生成派生参数
            ...
            parameters={'spc1_for_MAS': [[9, 3, 9, 3, 1, 'param_150']]},
            # 在给定上面的外扩参数下，mask的tag为不同的派生方式, 常规有上下左右四种派生方向 可以添加其他派生方式 在函数中定义
            para_mask_tag_map={
                ...
                'rectum_ring_LCR': ('rectum_ring_LCR',),
                # 注意必要有逗号  可以附加新的参数 ：'rectum_ring_l','rectum_ring_r','ring_include_boundry'
            },
            # 某些派生 ROI 需要额外参考的 mask
            # 例如 rectum 的派生需要 spc1
            extra_mask_name_map={
                # 'rectum_ring_LCR': 'spc1'
            },
            # 若派生名无法通过前缀推断 base ROI，可在此指定
            base_roi_map={
                # 'rectum_ring_LCR': 'rectum'
            },
        ),
        # 四、Debug / 中间结果检查
        debug=DebugConfig(
            enabled=False,  # True 时才会 dump 中间结果
            # 留空则自动用 out_dir/_debug_dump
            out_dir=r"",
            # 只 dump 指定 ID（空 = 不限制）
            only_ids=set(),
            # 只 dump 指定 ROI
...
if __name__ == "__main__":
    # 创建命令行解析器  这个脚本支持命令行参数
    parser = argparse.ArgumentParser(description="CT 图像指标计算")
    # 声明你支持的 3 个“启动模式参数”
    parser.add_argument("--config", type=str, default="", help="配置文件路径（json/yaml）。不提供则使用脚本内默认配置。")
    # 生成一个“模板配置文件”  运行结果：# 把 _default_config_like_original() 生成的配置# 转成 JSON# 写到 cfg.json# 立刻退出，不做任何计算 # 👉 这是你以后改参数最推荐的方式
    parser.add_argument("--dump-config", type=str, default="", help="把默认配置导出为 json 到指定路径，然后退出。")
    # 跑内置单元测试 不需要真实 CT / RS 数据# 只验证：# 工程结构没坏 # 派生 ROI / 指标插件接口是通的   用法： python ct_metrics_engineered_v7_fix.py --run-tests
    parser.add_argument("--run-tests", action="store_true", help="运行内置单元测试并退出。")
    # 真正解析命令行
    args = parser.parse_args()
    if args.run_tests:  # （最高优先级）：如果是跑测试，立刻执行并退出
        raise SystemExit(run_unit_tests())
    if args.dump_config:  # 如果是导出配置，也立刻执行并退出
        cfg = _default_config_like_original()
        os.makedirs(os.path.dirname(args.dump_config) or ".", exist_ok=True)
        with open(args.dump_config, "w", encoding="utf-8") as f:
            json.dump(cfg.to_dict(), f, ensure_ascii=False, indent=2)
        print(f"✅ 默认配置已导出: {args.dump_config}")
        raise SystemExit(0)
    if args.config:  # 真正“决定用哪套配置”  if是你提供了config
        cfg = AppConfig.from_file(args.config)
        # debug.out_dir 为空时，默认放到 out_dir/_debug_dump
        if cfg.debug.enabled and not cfg.debug.out_dir:
            cfg.debug.out_dir = os.path.join(cfg.io.out_dir, "_debug_dump")
    else:  # 否则，使用默认配置
        cfg = _default_config_like_original()
        if cfg.debug.enabled and not cfg.debug.out_dir:
            cfg.debug.out_dir = os.path.join(cfg.io.out_dir, "_debug_dump")
